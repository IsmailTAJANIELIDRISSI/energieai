  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-card">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-slate-900">Alertes Urgentes</h3>
      ...
      <div className="space-y-4 max-h-96 overflow-y-auto scrollbar-thin">
        {alerts.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-400">Tous les systèmes fonctionnent normalement</p>
          </div>
        ) : (
          alerts.map((alert) => (
            <div
              key={alert.id}
              className={`p-4 rounded-lg border transition-all duration-200 hover:shadow-md ${getAlertColor(alert.severity)}`}
            >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {alert.machine && (
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <span className="text-xs font-medium">{alert.machine}</span>
                    </div>
                  )}
                  {alert.action && (
                    <Button 
      ...
      {alerts.length > 0 && (
        <div className="mt-4 pt-4 border-t border-slate-200">
          <Button variant="outline" fullWidth iconName="ExternalLink" iconSize={16}>
