...
const EnergyChart = ({ 
  ...
  const formatTooltip = (value, name) => {
    if (name === 'consumption' || name === 'staticAudit') {
      return [`${value.toLocaleString('fr-FR')} kWh`, name === 'consumption' ? 'Consommation Réelle' : 'Audit Statique'];
    }
    if (name === 'cost') {
      return [`${value.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} MAD`, 'Coût'];
    }
    if (name === 'efficiency') {
  ...
  const getYAxisLabel = () => {
    switch (selectedMetric) {
      case 'consumption': return 'Consommation (kWh)';
      case 'cost': return 'Coût (MAD)';
      case 'efficiency': return 'Efficacité (%)';
      case 'co2': return 'Émissions CO₂ (kg)';
        ...
        {showComparison && (
          <div className="flex items-center space-x-4 text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-muted-foreground">Surveillance Dynamique</span>
            </div>
            <div className="flex items-center space-x-2">
